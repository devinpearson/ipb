// File: src/register-cli-commands.ts
// Registers all ipb subcommands on the root Commander program instance.
...
import { determineExitCode, withCommandContext } from './utils.js';

/** Dependencies required to register CLI commands (injected from the entrypoint). */
export interface RegisterCliCommandsDeps {
...
/** Shared API credential options for most commands. */
...
/**
 * Registers all subcommands, options, and actions on `program`.
 */
export function registerCliCommands(program: Command, deps: RegisterCliCommandsDeps): void {
    ...
    .action(
      withCommandContext('config edit', async (options: { profile?: string }) => {
        await runConfigEdit({
          ...
          defaultCredentialsFile: deps.credentialLocation.filename,
        });
      })
    );
